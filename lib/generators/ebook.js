...
Generator.prototype.finish = function() {
    ...
    .then(function() {
        ...
        var command = [
            ...
            path.join(that.options.output, 'index.pre.'+that.ebookFormat),
        ...
        that.book.log.info(that.book);

        var command = [
            'pdftk',
            path.join(that.options.output, 'index.pre.'+that.ebookFormat),
            'background',
            path.join(stringUtils.escapeSpaces(that.book.root), 'images', 'bg.pdf'),
            'output',
            path.join(that.options.output, 'index.'+that.ebookFormat)
        ].join(' ');

        that.book.log.info('post-processing ', that.ebookFormat, '....');
        ...
        var child = exec(command, function (error, stdout) {
            if (error) {
                ...
                if (error.code == 127) {
                    error.message = 'Something went wrong, skipping post-processing.';
                } else {
                    error.message = error.message + ' '+stdout;
                }
                return d.reject(error);
